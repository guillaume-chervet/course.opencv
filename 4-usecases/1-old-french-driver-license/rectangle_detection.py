...
class BackgroundExtractor:
    ...
    def extract_background(self, rectangle, nb_iterations=1, destination_path=""):
        ...
        if self.environment_mode == "development":
            cv2.imwrite(destination_path + "4_img_grab_cut_small.png", img2)
...
def extract_rectangle(target_img, destination_path="", environment_mode="production"):
    ...
    thresh = cv2.adaptiveThreshold(img_gray, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 15, -2)
    ...
    if environment_mode == "development":
        cv2.imwrite(destination_path + "2_border_replicate_added.png", target_img)
    img_small = target_img.copy()
    img_small, ratio = normalize_size(img_small, img_tmp_size)
    color = (0, 0, 255)
    ...
    if environment_mode == "development":
        copy = img_small.copy()
        cv2.rectangle(copy, pt1=(rectangle[0], rectangle[1]), pt2=(rectangle[2], rectangle[3]), color=color, thickness=2)
        cv2.imwrite(destination_path + "3_img_small_with_contour_for_grabcut.png", copy)
    background_extrator = BackgroundExtractor(target_img.copy(), img_small, environment_mode)
    ...
    if contour_img_small_extracted is not None:
        contour = contour_img_small_extracted
    ...
    if contour is not None:
        ...
        if environment_mode == "development":
            blue = (255, 0, 0)
            cv2.drawContours(img_foreground, [box], 0, blue, 20)
            cv2.imwrite(destination_path + "6_img_with_min_rectangle.png", img_foreground)
